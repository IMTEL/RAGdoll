...
@pytest.fixture
def mongodb_dao() -> MongoDBAgentDAO:
    """Create a MongoDB Agent DAO for testing."""
...
@pytest.fixture(autouse=True)
def cleanup_mongodb(mongodb_dao: MongoDBAgentDAO):
...
class TestMongoDBAgentDAOConnection:
    ...
    def test_is_reachable_success(self, mongodb_dao: MongoDBAgentDAO):
        """Test that DAO can successfully connect to MongoDB."""
        assert mongodb_dao.is_reachable() is True

    def test_database_configuration(self, mongodb_dao: MongoDBAgentDAO):
        """Verify correct database and collection configuration."""
        config = Config()
        assert mongodb_dao.db.name == config.MONGODB_DATABASE
        assert mongodb_dao.collection.name == config.MONGODB_AGENT_COLLECTION
...
class TestMongoDBAgentDAOCreate:
    ...
    def test_create_agent_success(
        self, mongodb_dao: MongoDBAgentDAO, sample_agent: Agent
    ):
        """Test successful agent creation."""
        created_agent = mongodb_dao.add_agent(sample_agent)

        assert created_agent == sample_agent
        assert created_agent.name == "Test MongoDB Agent"

        # Verify it's actually in the database
        agents_in_db = list(mongodb_dao.collection.find())
    ...
    def test_create_multiple_agents(
        ...
        mongodb_dao.add_agent(agent1)
        mongodb_dao.add_agent(agent2)
...
class TestMongoDBAgentDAORetrieve:
    ...
    def test_get_agents_empty(self, mongodb_dao: MongoDBAgentDAO):
        """Test getting agents from empty database."""
        agents = mongodb_dao.get_agents()
        assert agents == []

    def test_get_agents_single(self, mongodb_dao: MongoDBAgentDAO, sample_agent: Agent):
        """Test getting a single agent."""
        mongodb_dao.add_agent(sample_agent)
    ...
    def test_get_agent_by_id_success(
        ...
        result = mongodb_dao.add_agent(sample_agent)
        agent_id = str(result.id)

        # Retrieve by ID
        retrieved_agent = mongodb_dao.get_agent_by_id(agent_id)
    ...
    def test_get_agent_by_id_not_found(self, mongodb_dao: MongoDBAgentDAO):
        """Test retrieving non-existent agent returns None."""
        fake_id = str(ObjectId())
        agent = mongodb_dao.get_agent_by_id(fake_id)
        assert agent is None

    def test_get_agent_by_id_invalid_format(self, mongodb_dao: MongoDBAgentDAO):
        """Test retrieving agent with invalid ID format returns None."""
        invalid_id = "not-a-valid-objectid"
        agent = mongodb_dao.get_agent_by_id(invalid_id)
        assert agent is None
...
class TestMongoDBAgentDAOEdgeCases:
    ...
    def test_agent_with_empty_roles(self, mongodb_dao: MongoDBAgentDAO):
        ...
        mongodb_dao.add_agent(agent)
        agents = mongodb_dao.get_agents()

        assert len(agents) == 1
        assert agents[0].roles == []

    def test_agent_with_empty_document_access(self, mongodb_dao: MongoDBAgentDAO):
...
class TestMongoDBAgentDAOUpdate:
    ...
    def test_update_agent_success(
        ...
        result = mongodb_dao.add_agent(updated_agent)
        assert result.id == agent_id
        assert result.name == "Updated MongoDB Agent"
        # Check in DB
        db_agent = mongodb_dao.get_agent_by_id(agent_id)
    ...
    def test_update_agent_invalid_id_format(
        ...
        with pytest.raises(ValueError):
            mongodb_dao.add_agent(invalid_agent)
...
def test_add_agent_encrypts_api_key(mongodb_dao, sample_agent):
    """Test that add_agent encrypts the llm_api_key before storing."""
    original_key = sample_agent.llm_api_key

    # Add the agent
    saved_agent = mongodb_dao.add_agent(sample_agent)

    # Retrieve raw document from MongoDB
    agent_doc = mongodb_dao.collection.find_one({"_id": ObjectId(saved_agent.id)})
...
@pytest.mark.integration
def test_get_agent_by_id_decrypts_api_key(mongodb_dao, sample_agent):
    ...
    # Retrieve the agent
    retrieved_agent = mongodb_dao.get_agent_by_id(saved_agent.id)
...
@pytest.mark.integration
def test_get_agents_keeps_api_keys_encrypted(mongodb_dao, sample_agent):
    ...
    # Retrieve all agents
    all_agents = mongodb_dao.get_agents()
...
@pytest.mark.integration
def test_update_agent_re_encrypts_api_key(mongodb_dao, sample_agent):
    ...
    # Update the agent
    updated_agent = mongodb_dao.add_agent(saved_agent)

    # Retrieve raw document
    agent_doc = mongodb_dao.collection.find_one({"_id": ObjectId(updated_agent.id)})
