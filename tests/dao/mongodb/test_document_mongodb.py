...
class TestMongoDBDocumentDAO:
    ...
    def setup_and_teardown(self):
        ...
        self.document_dao = MongoDBDocumentDAO()
        # Clear the collection before and after each test
        self.document_dao.collection.delete_many({})
        yield
        self.document_dao.collection.delete_many({})

        # Drop the test database after tests
        self.document_dao.client.drop_database(self.document_dao.db.name)

    def test_is_reachable(self):
        """Test that DAO can reach MongoDB."""
        assert self.document_dao.is_reachable() is True
    ...
    def test_create_document(self):
        ...
        created = self.document_dao.create(doc)
        ...
        count = self.document_dao.collection.count_documents({})
        assert count == 1
    ...
    def test_create_same_name_different_agent(self):
        ...
        created1 = self.document_dao.create(doc1)

        doc2 = Document(
            name="shared_name.txt",
            agent_id="agent-2",
        )
        created2 = self.document_dao.create(doc2)
    ...
    def test_get_by_id(self):
        ...
        retrieved = self.document_dao.get_by_id(created.id)
    ...
    def test_get_by_id_not_found(self):
        ...
        result = self.document_dao.get_by_id("non-existent-id")
    ...
    def test_get_by_agent_id(self):
        ...
        self.document_dao.create(doc1)
        self.document_dao.create(doc2)
        self.document_dao.create(doc3)

        # Get documents for agent-1
        results = self.document_dao.get_by_agent_id("agent-1")
    ...
    def test_get_by_agent_id_empty(self):
        ...
        results = self.document_dao.get_by_agent_id("non-existent-agent")
        assert results == []
    ...
    def test_get_by_name_and_agent(self):
        ...
        retrieved = self.document_dao.get_by_name_and_agent("specific.txt", "agent-123")
    ...
    def test_get_by_name_and_agent_not_found(self):
        ...
        result = self.document_dao.get_by_name_and_agent("missing.txt", "agent-123")
        assert result is None
    ...
    def test_update_document(self):
        ...
        updated = self.document_dao.update(created)
    ...
    def test_delete_document(self):
        ...
        # Verify it exists
        assert self.document_dao.collection.count_documents({}) == 1

        # Delete the document
        self.document_dao.delete(created.id)

        # Verify it's gone
        retrieved = self.document_dao.get_by_id(created.id)
        assert retrieved is None
        assert self.document_dao.collection.count_documents({}) == 0

    def test_delete_nonexistent_document(self):
        """Test deleting a document that doesn't exist."""
        # Should not raise an error
        self.document_dao.delete("non-existent-id")
        assert self.document_dao.collection.count_documents({}) == 0

    def test_indexes_created(self):
        """Test that MongoDB indexes are created."""
        # Indexes should be created on initialization
        indexes = list(self.document_dao.collection.list_indexes())
    ...
    def test_agent_id_index_used(self):
        ...
        for i in range(10):
            ...
            self.document_dao.create(doc)

        # Query explanation should show index usage
        explain_result = self.document_dao.collection.find(
            {"agent_id": "agent-123"}
        ).explain()
