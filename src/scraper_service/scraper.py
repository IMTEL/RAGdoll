        ...
        try:
            ...
            if file_extension == ".pdf":
                ...
                if not elements or len(elements) == 0:
                    try:
                        logger.info(
                            "Trying PDF processing with OCR (ocr_only strategy)"
                        )
                        elements = partition_pdf(
                            filename=file_path,
                            strategy="ocr_only",
            ...
            if elements:
                logger.info(
                    f"Element types found: {[type(elem).__name__ for elem in elements[:5]]}"
                )
                logger.info(
                    f"Sample element content: {[str(elem)[:100] + '...' if len(str(elem)) > 100 else str(elem) for elem in elements[:2]]}"
                )
            else:
